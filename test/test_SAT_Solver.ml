...
let test_parse_simple_not_1 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(~a)") in
  let tree = Clause(Lit(Not (Var 'a'))) in
  assert (test = tree)
...
let test_parse_or_2 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(~a | b)") in
  let tree = Clause(Or(Not(Var 'a'), Lit(Var 'b'))) in
  assert (test = tree)

let test_parse_or_3 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(a | ~b)") in
  let tree = Clause(Or(Var 'a', Lit(Not(Var 'b')))) in
  assert (test = tree)

let test_parse_or_4 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(~a | ~b)") in
  let tree = Clause(Or(Not(Var 'a'), Lit(Not(Var 'b')))) in
  assert (test = tree)
...
let test_parse_and_2 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(~a) & (b)") in
  let tree = And(Lit(Not(Var 'a')), Clause(Lit(Var 'b'))) in
  assert (test = tree)

let test_parse_and_3 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(a) & (~b)") in
  let tree = And(Lit(Var 'a'), Clause(Lit(Not(Var 'b')))) in
  assert (test = tree)

let test_parse_and_4 () =
  let test = Satsolver_lib.Parser.parse (Satsolver_lib.Lexer.lexer "(~a) & (~b)") in
  let tree = And(Lit(Not(Var 'a')), Clause(Lit(Not(Var 'b')))) in
  assert (test = tree)
