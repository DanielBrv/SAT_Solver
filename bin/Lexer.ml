...
let rec lexer input pos tokens = 
  let length = String.length input in
    ...
    else if pos > length - 1 || pos < 0 then raise (Failure ("Illegal lexing Pos: " ^ string_of_int pos))
    else if is_whitespace (String.get input pos) then lexer input (pos + 1) tokens @ []
    else if length - 1 = pos then tokens @ [lexer_help input pos] 
    else lexer input (pos + 1) tokens @ [lexer_help input pos]
